 ...
 * 
 */
//TODO: This class is based on org.w3c.css.css.StyleSheetGenerator, its been 
//modified to allow the maven plugin to init velocity
public class ReportGenerator extends StyleReport {
    ...
    //TODO: init logic was in the static block moved it to init method,
    //allows a non singleton instance of a velocity engine, css report
    //generator uses a singleton, which could possibly cause problems with other plugins??
    static {
    ...
    /**
     * 
     * @param inVelocityEngine non single to instance
     */
    //TODO: clean up!
    public static void init(VelocityEngine inVelocityEngine) {
    	velocityEngine = inVelocityEngine; 
    	availableFormat = new Utf8Properties();
         try {
             java.io.InputStream f;
 	    f = ReportGenerator.class.getResourceAsStream(
 							   "format.properties");
             availableFormat.load(f);
             f.close();
         } catch (Exception e) {
        	 //TODO: clean up exception handling?
             System.err.println(ReportGenerator.class.getName()+
         ...
         try {
             java.io.InputStream f;
 	    f = CssParser.class.getResourceAsStream(
 							     "urls.properties");
         ...
         try {
             ...
             velocityEngine.addProperty(Velocity.RESOURCE_LOADER, "jar");
             velocityEngine.setProperty("jar." + Velocity.RESOURCE_LOADER + ".class",
 	       "org.apache.velocity.runtime.resource.loader.JarResourceLoader");
             URL path = ReportGenerator.class.getResource("/");
             //TODO: hack to get the name of the jar this class is in
             //The StyleSheetGenerator in the css jar expects the jar to be called
             //css-validator.jar, this causes a maven version e.g. css-validator-20100131.jar
             //to fail when run on the command line, should submit a path to that project.
             URL path2 = ReportGenerator.class.getProtectionDomain().getCodeSource().getLocation();
             
             if(path != null) {
                 velocityEngine.addProperty("file." + Velocity.RESOURCE_LOADER +
 				     ".path", path.getFile());
                 velocityEngine.setProperty( "jar." + Velocity.RESOURCE_LOADER+".path",
 				      "jar:" + path2); 
                 
             }
             System.out.println(StyleReportFactory.class.getProtectionDomain().getCodeSource().getLocation().toURI());
             velocityEngine.init();
         } catch(Exception e) {
    ...
    public ReportGenerator(String title, StyleSheet style, String document,
			       int warningLevel)
    {
        this(null, title, style, document, warningLevel);
    }

    public ReportGenerator(ApplContext ac, String title, StyleSheet style,
	...
	//TODO : remove this.
        ...
        try {
            template = velocityEngine.getTemplate("org/jvnet/mcvp/reporting/" + template_file);
    ...
    // TODO: would be nice to throw a specific exception here
    // This method is used instead of the print method (which the
    // StyleSheetGenerator class used).
    public void append(Writer out) throws Exception {
	StringWriter sw = new StringWriter();
	template.merge(context, sw);
	// basic white space stripping as the report can be very large with
	// warnings at level 2 and lots of stylesheets
	String x = sw.toString().replaceAll("\\s{2,}", " ");
	out.append(x);
	// template.merge(context, out);
    }
    
    //TODO: modification to make velocity context visible
